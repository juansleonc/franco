...
RSpec.describe 'V1::Tenants', type: :request do
  ...
  it 'creates, updates and deletes a tenant' do
    # create
    post '/v1/tenants', params: { tenant: { full_name: 'Pepe', email: 'pepe@example.com' } }, headers: auth_headers, as: :json
    expect(response).to have_http_status(:created)
    ten = JSON.parse(response.body)['data']

    # update
    patch "/v1/tenants/#{ten['id']}", params: { tenant: { phone: '123' } }, headers: auth_headers, as: :json
    expect(response).to have_http_status(:ok)
  ...
  it 'returns 422 on invalid create' do
    post '/v1/tenants', params: { tenant: { full_name: '' } }, headers: auth_headers, as: :json
  ...
  it 'returns 422 on invalid update' do
    ...
    ten = JSON.parse(response.body)['data']
    patch "/v1/tenants/#{ten['id']}", params: { tenant: { full_name: '' } }, headers: auth_headers, as: :json
    expect(response).to have_http_status(:unprocessable_content)
  end
